...
db.createCollection('employees', {collation: {locale: 'es', strength: 1}})

db.employees.insert({name: 'Juan', surname: 'Pérez'})

db.employees.createIndex({surname: 1})

db.employees.find({surname: 'Perez'}) // Aunque no se indique la colación se usa el índice
// porque la colación esta establecida a nivel de colección

db.employees.find({surname: 'Perez'} ,{collation: {locale: 'en', strength: 1}}) // e Aunque la
